...
- iPad/iPhoneでスキャン画面を開いた際に `NotAllowedError` が発生しカメラが起動しない場合: 設定アプリ → プライバシーとセキュリティ → カメラ → 「Safari」がオンになっているか確認してください。オフになっていると、サイト側の許可ダイアログで「許可」を選んでもカメラにアクセスできません。あわせて、Safariのアドレスバー左の「AA」アイコン →「Webサイトの設定」→「カメラ」がそのサイトに対して「拒否」になっていないかも確認してください。
